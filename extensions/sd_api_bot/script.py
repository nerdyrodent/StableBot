...
params = {
    ...
    'prompt_prefix': '',
    'negative_prompt': '(blurry, badly drawn, worst quality, low quality:1.2)',
    'side_length': 512,
    'restore_faces': False
}

SD_models = ['Unused'] # TODO: get with http://{address}}/sdapi/v1/sd-models and allow user to select
...
def input_modifier(string):
    ...
    commands = ['m5zxplyqekicjs']
    mediums =  ['m5zxplyqekicjs']
    subjects = ['m5zxplyqekicjs']
    lowstr = string.lower()

    # TODO: refactor out to separate handler and also replace detection with a regexp

    # trigger the generation if a command signature and a medium signature is found
    if any(command in lowstr for command in commands) and any(case in lowstr for case in mediums): 
        ...
        string = "Stable Diffusion Prompt Error Case"

        # the focus of the image should be on the sending character
        if any(target in lowstr for target in subjects):                                           
            string = "Image"
...
def get_SD_pictures(description):
    ...
    for img_str in r['images']:
        ...
        if not os.path.exists('extensions/sd_api_bot/outputs'):
           os.makedirs('extensions/sd_api_bot/outputs')

        if params['save_img']:
            output_file = Path(f'extensions/sd_api_bot/outputs/{pic_id:06d}.png')
            image.save(output_file.as_posix())
            pic_id += 1

        # lower the resolution of received images for the chat -
        # otherwise the log size gets out of control quickly with all the base64 values in visible history
...
def ui():
    ...
    if params['enable_SD_api']:
        print(f"enable_SD_api: {params['enable_SD_api']}")
        force_pic() # just force it
